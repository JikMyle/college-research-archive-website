...
<div {{ $attributes->merge(['class' => 'box-border flex flex-row items-center gap-3'])}}>
    ...
    <fieldset 
        {{ $input->attributes->only('class')->merge([
            'class' => 'flex w-full h-full relative border-input-border-light bg-transparent dark:border-input-border-dark dark:bg-transparent dark:text-text-dark border-2 rounded-xl  dark:placeholder:text-text-dark'
            ])
        }}>
